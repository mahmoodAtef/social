...
class ProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    late var uid = CacheHelper.getData(key: 'uid').then((value) => {print('uuuuuiiiiidddd $value',
    ),  AppCubit.get(context).getUserData(value)}).then((value) async{
   ...
   await   AppCubit.get(context).getFollowers(user);
     await AppCubit.get(context).getPosts(user);
    });
    print ('my uid is: $uid');
    AppCubit.get(context).currentTab = 0 ;
    return BlocConsumer<AppCubit, AppStates>(
      listener: (context, state) { user = AppCubit.get(context).userModel;},
                                ...
                                child: TabBar (
                                  ...
                                  [
                                    ...
                                    Padding
                                      ...
                                      child: Text('${AppCubit.get(context).following.length}'),
                                    ),
                                ],),
                                  Column (children:  [
                                    ...
                                    Padding(
                                      padding: const EdgeInsets.symmetric(vertical: 5),
                                      child: Text('${AppCubit.get(context).followers.length}'),
                                    ),
                                  ],),
                                  Column (children: [
                                    ...
                                    Padding(
                                      padding: const EdgeInsets.symmetric(vertical: 5),
                                      child: Text('${AppCubit.get(context).posts.length}'),
                                    ),
                                  ],),
                                   //SizedBox(width: 0,)
